...
export function ThemeSelector() {
  ...
  return (
    <div className="w-full sm:w-auto">
      ...
      >
        {options.map(({ value, label, icon: Icon }) => (
          <button
            ...
            className={`flex min-w-0 items-center justify-center gap-1.5 rounded-lg border px-3 py-2 font-medium text-xs transition-colors sm:px-2.5 sm:py-1.5 ${
              activeTheme === value
      ...
      <p className="mt-2 text-right text-[11px] text-muted-foreground">{helperText}</p>
    </div>
  );
}
