...
export function AvatarVariationsPanel({
  ...
  useEffect(() => {
    const isGenerating = variations.some(
      (v) => v.status === VARIATION_STATUS.GENERATING || v.status === VARIATION_STATUS.PENDING,
    );
    if (!isGenerating) {
  ...
  const canUseSelectedVariation = selectedVariation?.status === VARIATION_STATUS.COMPLETED;
              ...
              >
                <div className="flex gap-3 overflow-x-auto pb-2">
                  {variations.map((variation) => {
                    ...
                    const isClickable =
                      variation.status === VARIATION_STATUS.COMPLETED && !!onVariationClick;
                    return (
                        ...
                        >
                          {variation.status === VARIATION_STATUS.COMPLETED && variation.imagePath ? (
                            <Image
                              ...
                              unoptimized
                            />
                          ) : variation.status === VARIATION_STATUS.FAILED ? (
                          ...
                          <div className="absolute top-1 right-1 flex flex-col gap-1">
                            ...
                            {variation.status !== VARIATION_STATUS.PENDING &&
                              variation.status !== VARIATION_STATUS.GENERATING && (
                                <button
                                  type="button"
                                  onClick={(e) => {
                                    e.stopPropagation();
